...
1. Send `OVDB-Page-Size: 1..1000` with a DTQL document whose `limit` and
   ...
   pending.
2. The response is `{"records":[...],"nextPageToken":"...","snapshotExpiresAt":"..."}`.
...
capture, on database unmount, or on server shutdown; stale files from a crash
...
allows both paging headers when the origin is allowed.
